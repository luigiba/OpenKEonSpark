...
def get_gpus(num_gpu=1, worker_index=-1):
  ...
  while len(free_gpus) < num_gpu and retries < MAX_RETRIES:
    ...
    smi_output = subprocess.check_output(["nvidia-smi", "--format=csv,noheader,nounits", "--query-gpu=gpu_uuid,memory.free"]).decode()
    
    #logging.debug("busy GPUs:\n{0}".format(smi_output))
    print("Smi output: {}".format(smi_output))
    
    #busy_uuids = [x for x in smi_output.split('\n') if len(x) > 0]
    smi_output_arr = [x for x in smi_output.split('\n') if len(x) > 0]
    busy_uuids = [x.split(",")[0].strip() for x in smi_output_arr if int(x.split(",")[1].strip()) == 0]
