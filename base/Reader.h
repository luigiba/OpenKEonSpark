...
void importTrainFiles() {
	...
	if (fin == nullptr) {
		std::cout << '`' << inPath << "relation2id.txt" << '`' << " does not exist" << std::endl;
		return;
	}
	...
	if (fin == nullptr) {
		std::cout << '`' << inPath << "entity2id.txt" << '`' << " does not exist" << std::endl;
		return;
	}
    ...
    **/
    ...
    if (fin_ != nullptr){
        std::cout << '`' << inPath << "batch2id.txt" << '`' << " founded!" << std::endl;
        tmp = fscanf(fin_, "%ld", &newBatchTotal);
        std::cout << "The total number of new batch triples is: " << newBatchTotal << std::endl;
        fclose(fin_);
    }
...
void importTestFiles() {
	...
	if (f_kb1 == nullptr) {
		std::cout << '`' << inPath << "test2id.txt" << '`' << " does not exist" << std::endl;
		return;
	}
	...
	if (f_kb3 == nullptr) {
		std::cout << '`' << inPath << "valid2id.txt" << '`' << " does not exist" << std::endl;
		return;
	}
...
void importTypeFiles() {
	...
	if (f_type == nullptr) {
		std::cout << '`' << inPath << "type_constrain.txt" << '`' << " does not exist" << std::endl;
		return;
	}
